...
Strategy::Strategy() {
    this->epsilon = 1.0;
    vector x(11, vector<vector<double> >(33, vector<double>(2, 0.1)));
    vector y(11, vector<vector<double> >(33, vector<double>(2, 0.1)));
    this->QValsNoAce = x;
    this->QValsWithAce = y;
}
    ...
    if (r == 0.0) {
        if (hadUsableAce) {
            qValue = QValsWithAce[dealerShow][playerHad][action];
                estimatedQVal =
                        qValue + (alpha * (r + (gamma * (QValsWithAce[dealerShow][(playerHas)][
                                                    1])) - qValue));
        } else {
            qValue = QValsNoAce[dealerShow][playerHad][action];
                estimatedQVal =
                        qValue + (alpha * (r + (gamma * (QValsNoAce[dealerShow][(playerHas)][
                                                    1])) - qValue));
        }
        if (hadUsableAce) {
            QValsWithAce[dealerShow][playerHad][action] = estimatedQVal;
        } else {
            QValsNoAce[dealerShow][playerHad][action] = estimatedQVal;
        }
    }
    if (r == 1.0) {
        if (hasUsableAce) {
...
void Strategy::setEpsilon(int i) {
    this->epsilon = i;
}
